class AppRoutes {
  static String signupPage = "/SignupPage";
  static String sigInPage = "/SigInPage";
  static String homePage = "/homePage";
  static String newOrderPage = "/newOrderPage";
  static String orderListPage = "/orderListPage";
  static String pendingOrderPage = "/pendingOrderPage";
  static String deliveredPage = "/deliveredPage";
  static String returnOrderPage = "/returnOrderPage";
  static String newServicePage = "/newServicePage";
  static String pendingServicePage = "/pendingServicePage";
  static String completeServicePage = "/completeServicePage";
}
